<?php class Simpeg_model extends CI_Model {
	...
	public function getRiwayatJabatanPensiun($nip)
	{
		...
		public function getRiwayatOrganisasiPensiun($nip)
		{
			$DB2 =$this->load->database('simpegRef', TRUE);
...
WHERE NIP = '$nip'";
	...
	public function getRiwayatJasaPensiun($nip)
	{
		$DB2 =$this->load->database('simpegRef', TRUE);
		$querySQL = "SELECT nip, NBINTANG, skNomor, skDate, tahun, AOLEH
		FROM revReferenceSimpeg.penghargaan3
		WHERE nip = '$nip'";
	...
	public function getRiwayatDPPPPensiun($nip)
	{
		$DB2 =$this->load->database('simpegRef', TRUE);
		$querySQL = "SELECT NIP, THNILAI, NSETIA, NPRES, NTJAWAB, NTAAT, NJUJUR, NKSAMA, NPKARSA, NPIMPIN, NTOTAL, NRATA
		FROM revReferenceSimpeg.rdppp
	...
	public function getRiwayatPenilaianKerjaPensiun($nip)
	{
		$DB2 =$this->load->database('simpegRef', TRUE);
		$querySQL = "SELECT NIP, NSKP, NPelayanan, NItegritas, NKomitmen, NDisiplin, NKerjasama, NKepemimpinan, NJumlah, NRataRata, NPrilaku, TotalPrestasi
FROM revreferencesimpeg.prestasikerja
WHERE NIP = '$nip'";
		...
		{ $count = 1;
			foreach($query->result() as $row)
			{
				$data['NIP']=$row->NIP;
				$data['NSKP']=$row->NSKP;
				$data['NPelayanan']=$row->NPelayanan;
				$data['NItegritas']=$row->NItegritas;
				$data['NKomitmen']=$row->NKomitmen;
				$data['NDisiplin']=$row->NDisiplin;
				$data['NKerjasama']=$row->NKerjasama;
				$data['NKepemimpinan']=$row->NKepemimpinan;
				$data['NJumlah']=$row->NJumlah;
				$data['NRataRata']=$row->NRataRata;
				$data['NPrilaku']=$row->NPrilaku;
				$data['TotalPrestasi']=$row->TotalPrestasi;
				array_push($stackData,$data);
			}
			$query->free_result();
			return $stackData;
	...
	public function getRiwayatHukumanPensiun($nip)
	{
		$DB2 =$this->load->database('simpegRef', TRUE);
		$querySQL = "SELECT rh.nipBaru, rh.jenisHukuman, rh.DESHUKUM, rh.skNomor, rh.skTanggal, rh.KPEJ, rp.npej, rh.hukumanTanggal
FROM revReferenceSimpeg.hukumandisiplin rh
INNER JOIN revReferenceSimpeg.pejabatMenetapkan rp on rp.kpej = rh.KPEJ
WHERE rh.nipBaru = '$nip'";
	...
	public function getRiwayatCutiPensiun($nip)
	{
		$DB2 =$this->load->database('simpegRef', TRUE);
...
WHERE nip = '$nip'";
	...
	public function getRiwayatTLNPensiun($nip)
	{
		$DB2 =$this->load->database('simpegRef', TRUE);
...
WHERE nip = '$nip'";
	...
	public function getRiwayatBahasaPensiun($nip)
	{
		$DB2 =$this->load->database('simpegRef', TRUE);
...
WHERE NIP = '$nip'";
	...
	public function getRiwayatPendidikanUmumPensiun($nip)
	{
		$DB2 =$this->load->database('simpegRef', TRUE);
		$querySQL = "SELECT rp.nip, rt.NJUR, rp.npdum, rp.namaSekolah, rp.tempat, rp.nkepsek, rp.nomorIjazah, rp.tglTahunLulus, rp.isPendidikanTerakhir
FROM revReferenceSimpeg.pendidikan rp
LEFT JOIN revreferencesimpeg.tpu rt on rt.KJUR LIKE CONCAT(rp.ktpu, '00000')
WHERE rp.NIP = '$nip'
ORDER BY rp.tglTahunLulus DESC";
	...
	public function getRiwayatPendidikanFungsionalPensiun($nip)
	{
		$DB2 =$this->load->database('simpegRef', TRUE);
		$querySQL = "SELECT rr.NIP, rr.kFungStrTek, rr.namaDiklat, rr.TEMPAT, rr.PAN, rr.ANGKATAN, rr.TMULAI, rr.TAKHIR, rr.JAM, rr.NSTTPP, rr.TSTTPP, rr.ISAKHIR
...
ORDER BY rr.TAKHIR DESC";
	...
	public function getRiwayatPendidikanStrukturalPensiun($nip)
	{
		$DB2 =$this->load->database('simpegRef', TRUE);
		$querySQL = "SELECT rr.NIP, rr.kFungStrTek, rr.namaDiklat, rr.TEMPAT, rr.PAN, rr.ANGKATAN, rr.TMULAI, rr.TAKHIR, rr.JAM, rr.NSTTPP, rr.TSTTPP, rr.ISAKHIR
FROM revReferenceSimpeg.riwayatdiklat rr
...
ORDER BY rr.TAKHIR DESC";
	...
	public function getRiwayatPendidikanTeknisPensiun($nip)
	{
		$DB2 =$this->load->database('simpegRef', TRUE);
		$querySQL = "SELECT rr.NIP, rr.kFungStrTek, rr.namaDiklat, rr.TEMPAT, rr.PAN, rr.ANGKATAN, rr.TMULAI, rr.TAKHIR, rr.JAM, rr.NSTTPP, rr.TSTTPP, rr.ISAKHIR
FROM revReferenceSimpeg.riwayatdiklat rr
...
ORDER BY TAKHIR DESC";
	...
	public function getRiwayatPendidikanSeminarPensiun($nip)
	{
		$DB2 =$this->load->database('simpegRef', TRUE);
...
ORDER BY TAKHIR DESC";
	...
	public function getRiwayatPendidikanKursusPensiun($nip)
	{
		$DB2 =$this->load->database('simpegRef', TRUE);
...
WHERE nip = '$nip'
ORDER BY tanggalAkhirKursus DESC";
